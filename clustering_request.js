...
async function handle_master() {
    ...
    for (var i = 0; i < totalCPUs; i++) {
        ...
        worker.on('message', function (msg) {
            if (msg.task_id && current_task < MAX_NBR_TASK) {
                let new_task_id = current_task++;
                console.log(`## Master => ${msg.worker_id}: ok, new task_id=${new_task_id}`)
                worker.send({COLLECTION_ID: COLLECTION_ID, task_id: new_task_id, task_size: task_size, worker_id: msg.worker_id})
            }
        });
        workers.push(worker)
    }
    ...
    for (let i=0; i<totalCPUs; i++) {
        ...
        workers[i].send({task_id: current_task++, task_size: task_size/(i+1)})
    }
    
    while (true) {
        if (current_task > MAX_NBR_TASK) {
            console.log("## Sent req_stop_collecting_stat")
            req_stop_collecting_stat(session_id).catch(e => { console.log({ error: e }) })
            break
        }
        await sleep_ms(100)
    }
